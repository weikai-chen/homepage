...
author = ["陈伟凯"]
date = 2021-02-03
lastmod = 2021-02-08T00:06:11+08:00
tags = ["概率", "统计"]
categories = ["Notes"]
type = "docs"
draft = false
...
[menu.PSET]
  weight = 2001
  identifier = "积分--integration"
+++

## 引言 {#引言}

这是本系列的第一部分，在进入概率论之前，我们会先讨论勒贝格(Lebesgue) 积分和简单的测度论。

首先我们要问，我们已经有了微积分中学到的黎曼积分，为什么要引入另外的一种积分？
我们知道黎曼积分有一些不错的性质，如微积分基本定理等。另外，如果\\(f\_n:[a,b]\to \mathbb{R}\\) 一致收敛到\\(f\\)的话，还有“ **积分的极限等于极限的积分** ”。但是黎曼积分也有一些不足：

-   有一些我们觉得应该可以积分的函数却不可积，比如\\(f=\chi\_{\mathbb{Q}\cap [0,1]}\\)，即\\([0,1]\\)上有理数集的特征函数，它在有理数上取 1，无理数取 0. 由于有理数在实数集上是稠密的(dense)，不管划的小区间有多小，上面总是同时存在有理数和无理数，因此，在每个小区间上取出来的函数值或是 1 或是 0，所以有达布上积分为 1，达布下积分为 0，二者不等，故黎曼不可积
-   如果没有一致收敛的条件，没有办法保证\\[ f\_n\to f\Longrightarrow \int f\_n(x)dx \to \int f(x) dx\\]
-   黎曼积分不能推扩到其它情形，如在我们要讨论的概率论中，我们知道期望对应的就是积分，但是如果随机变量不是单纯的离散或连续型的，而是一般情形的话，只用黎曼积分就不好处理

勒贝格积分就比较好的处理了上述的困难，类似于通过引入无理数使得有理数集完备化，勒贝格可积函数空间也是对黎曼可积函数空间的完备化，其构造的关键想法是

1.  不是通过积分来定义集合的测度（长度），而是直接定义的一个集合的测度，再由此定义积分
2.  不分割定义域然后用阶梯函数逼近，而是分割值域然后用“简单函数”逼近。就像在加总班级考试成绩的时候，我不是一个一个相加，而是做成一个频数分布直图，知道各个分数各个多少人，然后汇总。
...
接下来的任务是构造一个满足上述性质的函数。要满足性质 2 故定义区间测度等于其长度，有了区间的测度后对其它集合\\(A\subseteq \mathbb{R}\\)就用覆盖它的区间的测度来逼近

\begin{equation}
m^{\*}(A)=\inf \\{\sum\_{n=1}^{\infty}m(I\_{n}): A\subseteq \bigcup\_{n=1}^{\infty}I\_{n}\\}
\end{equation}

我们称上述函数为 **外测度** (outer measure），可以证明这个函数满足上述性质 1-4 以及单调性和有限可加性。如果它满足可数可加性的话，就是我们要的测度。可惜的是，对有一些奇怪的集合这个性质并不满足。[^fn:1]

不过这种奇怪集合的构造依赖于选择公理（Axiom of Choice），因而我们转而对测度函数的定义域进行一定的限制以排除这种奇怪的集合，剩下的集合称之为勒贝格可测。可以通过不同的方式进行限定，比如定义
...
最后把外测度\\(m^{\*}:\mathcal{P}(\mathbb{R})\to [0,\infty]\\)限制在
\\(\mathcal{M}\_{\mathrm{Leb}}\\)上，记为\\(m:\mathcal{M}\_{\mathrm{Leb}}\to[0,\infty]\\)，则可以证明它具有可列可加性，从而满足所有测度的性质。


## 测度空间与可测函数 {#测度空间与可测函数}

[^fn:1]: 例如把加法群\\((\mathbb{R},+)\\)分割成其子群\\((\mathbb{Q},+)\\)的陪集（cosets），再从每一个陪集（或等价类）中 **选出** 一个元素，构成一个集合，可以证明该集合不可测。
